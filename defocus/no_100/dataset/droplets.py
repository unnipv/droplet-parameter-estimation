...
class ScaleStuff(object):
    ...
    def __call__(self, item):
        ...
        return torch.Tensor([dia_normalized, pos_normalized])
...
#Uncomment this if resampling is required in case of very large dataset
        self.csv = self.csv.sample(frac = 0.4, random_state = 42)
    ...
    def __getitem__(self, idx):
        ...
        label = [float(self.csv.iloc[idx, 1]), float(self.csv.iloc[idx, 2])]
        if self.transform:
